...
export const BottomNavigation = ({ step, setStep }: BottomNavigationProps) => {
	return (
		...
		<div className="flex justify-between px-4">
			<div>
				{typeof step === "number" && step > 1 && (
					<Button
						onClick={() => setStep((prev) => (typeof prev === "number" ? prev - 1 : 1))}
					>
						Previous
					</Button>
				)}
			...
			<div>
				{typeof step === "number" && step < 3 && (
					<Button
						onClick={() => setStep((prev) => (typeof prev === "number" ? prev + 1 : 3))}
					>
						Next
					</Button>
				)}
				{step === 3 && <Button onClick={() => setStep("complete")}>Complete</Button>}
			</div>
		</div>
	);
};
...
export const TopNavigation = ({ step }: TopNavigationProps) => {
	return (
				...
				<div className="flex items-center md:w-64">
					<div className="w-full bg-gray-200 rounded-full mr-2">
						<div
							...
							style={{
								width: `${((typeof step === "number" ? step : 3) * 100) / 3}%`,
							}}
						></div>
					</div>
				...
				<div id="step1">
					...
					<div className="mb-5">
						...
						<Input
							type="text"
							id="firstname"
							className="w-full px-4 py-3 rounded-lg shadow-sm focus:shadow-outline text-gray-600 font-medium"
